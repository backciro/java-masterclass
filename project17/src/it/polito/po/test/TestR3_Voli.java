...
public class TestR3_Voli extends TestCase {
	...
	public void testVolitoString() {
		try{
			al.addAereo("734", 120);
			al.addVolo("AZ321", "734", "Torino", "Paris", "Martedi");
			al.addVolo("AZ321", "345", "Torino", "Paris", "Giovedi");
			fail("Il codice non dovrebbe essere valido");
		}catch(InvalidCode ic){
		}
		try {
			lu.addAereo("734", 150);
			lu.addVolo("AZ321", "734", "Torino", "Paris", "Martedi");
		}catch(InvalidCode ic){
			fail("Il codice dovrebbe essere valido");
		}
		for(Volo v : lu.getVoli())
			assertEquals("Lufthansa;AZ321;734;Torino;Paris;Martedi;150",v.toString());
	}
	
	public void testVoliOrdinati() {
		try{
			al.addAereo("734", 120);
			al.addVolo("AZ322", "734", "Torino", "Paris", "Martedi");
			al.addVolo("AZ321", "734", "Paris","Torino" , "Lunedi");
			al.addVolo("AZ324", "734", "Torino", "Paris", "Giovedi");
			al.addVolo("AZ323", "734", "Paris","Torino" , "Mercoledi");
		}catch(InvalidCode ic){
		}

			assertEquals(al.getVoli().get(0).toString(), "Alitalia;AZ321;734;Paris;Torino;Lunedi;120");
			assertEquals(al.getVoli().get(1).toString(), "Alitalia;AZ322;734;Torino;Paris;Martedi;120");
			assertEquals(al.getVoli().get(2).toString(), "Alitalia;AZ323;734;Paris;Torino;Mercoledi;120");
			assertEquals(al.getVoli().get(3).toString(), "Alitalia;AZ324;734;Torino;Paris;Giovedi;120");
	}
	...
	public void testVoliVuoti() {
	   ...
	   public void testArriviPartenzetoString() {
	        try{
	            lu.addAereo("734", 150);
	            lu.addVolo("LH359", "734", "Torino", "Paris", "Martedi");
	        }catch(InvalidCode ic){
	            fail("Il codice dovrebbe essere valido");
	        }
	        for(Volo v : am.getAeroporto("Torino").getPartenze())
	            assertEquals("Lufthansa;LH359;734;Torino;Paris;Martedi;150",v.toString());
	    ...
	    public void testArriviOrdinati() {
	        try{
	            lu.addAereo("734", 150);
	            lu.addVolo("LH359", "734", "Torino", "Paris", "Martedi");
	            lu.addVolo("LH357", "734", "Torino", "Paris", "Lunedi");
	        }catch(InvalidCode ic){
	            fail("Il codice dovrebbe essere valido");
	        }
	        List<Volo> p = am.getAeroporto("Paris").getArrivi();
	        assertEquals(p.get(0).toString(),"Lufthansa;LH357;734;Torino;Paris;Lunedi;150" );
	        assertEquals(p.get(1).toString(),"Lufthansa;LH359;734;Torino;Paris;Martedi;150" );
	    }
